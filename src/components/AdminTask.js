...
export default function AdminTask() {
    ...
    const filtertask = (event) => {
        ...
        axios.get('http://localhost:8000/filtertaskadmin/' + field + '/' + value).then(
            (response) => {
                dispatch(showTasks({
                    tasks: response.data,
                    id: user.currentUser.user.id
    ...
    const searchtask = (event) => {
        ...
        axios.get('http://localhost:8000/searchtask/' + field).then(
            (response) => {
                dispatch(showTasks({
                    tasks: response.data,
                    id: user.currentUser.user.id
    ...
    function sortTask(event) {
        ...
        if (order != 'None' && sort != "") {
            console.log(sort);
            console.log(order);
            axios.get('http://localhost:8000/sorttaskadmin/' + sort + '/' + order).then(
                (response) => {
